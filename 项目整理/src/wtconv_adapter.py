...
def _import_wtconv2d():
    ...
    _proj_dir   = os.path.dirname(_this_dir)                          # project root/
    _root_dir   = os.path.dirname(_proj_dir)
    _candidates = [
        os.path.join(_proj_dir, "WTConv"),                            # organized project/WTConv/
        os.path.join(_proj_dir, "models", "external", "WTConv"),      # organized project/models/external/WTConv/
        os.path.join(_root_dir, "WTConv"),                            # original project_root/WTConv/
    ]
    _wtconv_dir = next((p for p in _candidates if os.path.isdir(p)), _candidates[0])

    if not os.path.isdir(_wtconv_dir):
        raise ImportError(
            f"找不到 WTConv 仓库目录，已尝试: {_candidates}\n"
            "请确认：\n"

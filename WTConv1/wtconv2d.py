...
class WTConv2d(nn.Module):
    """
    2维小波变换卷积层 (Wavelet Transform 2D Convolution)
    ...
    Shape:
        - Input: (N, C, H, W)
        - Output: (N, C, H//stride, W//stride) (stride>1时) 或 (N, C, H, W) (stride=1时)
    """
    def __init__(
        self,
        in_channels: int,
        ...
        # 手动计算same padding（兼容所有PyTorch版本）
    ...
    def _wavelet_forward_level(self, x: torch.Tensor, level_idx: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Size]:
        """单层级2D小波变换前向处理"""
        curr_shape = x.shape
        # 补零确保宽高均为偶数
        pad_h = 1 if curr_shape[2] % 2 != 0 else 0
        pad_w = 1 if curr_shape[3] % 2 != 0 else 0
        if pad_h > 0 or pad_w > 0:
            x = F.pad(x, (0, pad_w, 0, pad_h))  # (left, right, top, bottom)
        
        # 2D小波变换
        x_wt = wavelet.wavelet_2d_transform(x, self.wt_filter)
        x_ll = x_wt[:, :, 0, :, :]  # 低频分量
        
        # 小波域卷积处理
        batch, chan, coeff, h, w = x_wt.shape
        x_wt_reshaped = x_wt.reshape(batch, chan * 4, h, w)
        x_wt_reshaped = self.wavelet_scale[level_idx](self.wavelet_convs[level_idx](x_wt_reshaped))
        x_wt_processed = x_wt_reshaped.reshape(batch, chan, coeff, h, w)
        
        return x_wt_processed[:, :, 0, :, :], x_wt_processed[:, :, 1:4, :, :], curr_shape
    ...
    def _inverse_wavelet_level(
        ...
        x_iwt = wavelet.inverse_2d_wavelet_transform(x_cat, self.iwt_filter)
        # 裁剪回原始形状
        x_iwt = x_iwt[:, :, :orig_shape[2], :orig_shape[3]]
        return x_iwt
